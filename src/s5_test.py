...
SRC_WEBCAM_MAX_FPS = float(cfg["settings"]["src_webcam_max_framerate"])

# If data_type is video, set the sampling interval.
# For example, if it's 3, then the video will be read 3 times faster.
SRC_VIDEO_SAMPLE_INTERVAL = int(cfg["settings"]["src_video_sample_interval"])

# Openpose settings
OPENPOSE_MODEL = cfg["settings"]["openpose_model"]
OPENPOSE_IMG_SIZE = cfg["settings"]["openpose_img_size"]
...
def draw_result_img(img_disp, humans, dict_id2skeleton,
    ...
    img_disp = cv2.resize(img_disp, dsize=(desired_cols, DESIRED_ROWS))
