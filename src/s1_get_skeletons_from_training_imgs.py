'''
Based on a `valid_images.txt`, read training images from the folder.
...
# This txt will store image info, such as index, action type, filename, etc.
...
if __name__ == "__main__":
    ...
    for ith_img in range(num_total_images):
        ...
        filename = lib_io.int2str(ith_img, idx_len=5) + ".txt"
        lib_io.save_skeletons(
            DST_DETECTED_SKELETONS_FOLDER + filename,
            skels_to_save)

        filename = lib_io.int2str(ith_img, idx_len=5) + ".jpg"
        cv2.imwrite(
            DST_VIZ_IMGS_FOLDER + filename,
            img_disp)  # Save the visualized image for debug.
